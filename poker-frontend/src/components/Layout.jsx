...
//
// The import itself is app-wide state (ImportContext), not Layout's: it has
// to outlive this component, since navigating to another page unmounts and
// remounts the whole layout while the import keeps running. The status card
// lives outside Layout too, in <ImportStatus>.
const IMPORT_CTA_LABEL = 'Import hands';

export function Layout({ title, subtitle, ctaLabel, ctaIcon, onCta, ctaMenu, children }) {
  const isImportPage = ctaLabel === IMPORT_CTA_LABEL;
  const [isDraggingFile, setIsDraggingFile] = useState(false);
  const { uploadStatus, uploadDroppedFiles } = useImport();
